...
function cm_makeStyle(){
	...
	for(i=0;i<this.l.length;i++){
		if(i==0){if(bw.ns4){w="width:" +this.l[0].width+";"; this.l[i].style=""} else w=""; str+='.cl'+this.name+i+'{position:absolute; '+w+' background-color:transparent; color:'+this.l[i].tc+';'+this.l[i].style+';visibility:inherit; z-index:'+zindex+'}\n'}
		else{st='position:absolute; '+this.l[i].style+'; visibility:inherit; z-index:'+zindex; this.ns6style[i]=st}
	...
	if(this.usebar){
		...
		st='z-index:80; position:absolute; background-color:transparent; layer-background-color:transparent'
		if(!bw.ns6) str+='#div'+this.name+'Bar{'+st+'}\n'; else this.ns6styleb[this.ns6styleb.length]=st
		if(this.barinheritborder){str+='#div'+this.name+'Barb{z-index:85; position:absolute; background-color:transparent; layer-background-color:transparent}\n'}
	}
...
oCMenu.level[0].textcolor="#888888" 
oCMenu.level[0].hovercolor="#646464" 
...
oCMenu.level[2].bgcoloroff="#F1F1F1"
oCMenu.level[2].bgcoloron="#F8F8F8"
oCMenu.level[1].textcolor=""
...
oCMenu.level[3].bgcoloroff="#F8F8F8"
oCMenu.level[3].bgcoloron="#F1F1F1"
oCMenu.level[1].textcolor=""
...
oCMenu.makeMenu('top0','','<img src=./images/STsm.gif  >','index.php?OMode=SSSC','parent')
	oCMenu.makeMenu('sub00','top0','๏ อารัมภบท','index.php','parent')
	oCMenu.makeMenu('sub01','top0','๏ สารบัญ','index.php?OMode=CISC','parent')
		oCMenu.makeMenu('sub011','sub01','<form><input type=text name=search></form>','','parent',160,0)
	oCMenu.makeMenu('sub02','top0','๏ ค้นพระไตรปิฎก','index.php?OMode=SSSC','parent')
		oCMenu.makeMenu('sub021','sub02','- สะเตมี : ค้นพระไตรปิฎก','index.php?OMode=SSSC','parent',160,0)
		oCMenu.makeMenu('sub022','sub02','- สะเตมี : บาลี - ไทย','index.php?OMode=PI&bdr=off','parent',160,0)
		oCMenu.makeMenu('sub023','sub02','- สะเตมี : เรียนพระไตรปิฎก','index.php?OMode=TI&bdr=off','parent',160,0)
		oCMenu.makeMenu('sub024','sub02','- สะเตมี : ทุกระบบ','index.php?OMode=AI&bdr=off','parent',160,0)
	oCMenu.makeMenu('sub03','top0','๏ คู่มือ','index.php?OMode=GC','parent')
oCMenu.makeStyle(); 